...
export const getProduct=(keyword="",currentPage=1,price=[0,25000],category,ratings=0)=>async (dispatch)=>{
   try{
     dispatch({type:ALL_PRODUCT_REQUEST}); //{type:ALL_PRODUCT_REQUEST}:- "action" object in reducers
...
export const getProductDetails=(id)=>async (dispatch)=>{
   try{
     dispatch({type:PRODUCT_DETAILS_REQUEST}); //{type:ALL_PRODUCT_REQUEST}:- "action" object in reducers
